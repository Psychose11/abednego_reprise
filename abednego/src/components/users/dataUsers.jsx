...
export default function EnhancedTable() {
  ...
  const visibleRows = React.useMemo(() => {
    const filteredProspecteurs = stableSort(
      ...
      const phoneMatch = prospecteur.phone.toLowerCase().includes(searchLower);
      const nomMatch = prospecteur.nom.toLowerCase().includes(searchLower);
      const prenomMatch = prospecteur.prenom
        .toLowerCase()
        .includes(searchLower);

      // Utilisez une logique OR pour inclure les lignes correspondant à l'un des critères
      return (
        usernameMatch || mailMatch || phoneMatch || nomMatch || prenomMatch
      );
    });
              ...
              {visibleRows.map((prospecteur, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">{prospecteur.motDepasse}</TableCell>
                    <TableCell align="left">{prospecteur.mail}</TableCell>
                    <TableCell align="left">{prospecteur.phone}</TableCell>
                    <TableCell align="left">{prospecteur.nom}</TableCell>
                    <TableCell align="left">{prospecteur.prenom}</TableCell>
                  </TableRow>
                );
              })}
              {emptyRows > 0 && (
